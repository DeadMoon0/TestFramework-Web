...
<key_concepts>
    ...
    A database row has a key and a lifecycle, so it is an artifact, not a step. Three ways exist to put one in front of a test: SetupArtifact plus AddArtifact creates it, RegisterArtifact adopts an existing row by key, and FindArtifact locates one by predicate. All three are removed on teardown - discovery is not read-only. Chain MarkReadonly() onto the declaring call for a row the test only reads; teardown then records that it passed over it as information rather than as a failure.
